...
struct HitInfo
{
	...
	HitInfo() { Init(); }
	void Init() { z = BIGFLOAT; node = NULL; front = true; mtlID = 0; uvw.Set(0.5f, 0.5f, 0.5f); duvw[0].Zero(); duvw[1].Zero(); }
};
...
public:
	...
	public:
		...
		virtual Color directShade(const Ray &ray, const HitInfo &hInfo, const LightList &lights) const{ return Color(0, 0, 0); }
		virtual Color photonShade(const Ray &ray, const HitInfo &hInfo) const{ return Color(0, 0, 0); }
		virtual Color reflectShade(const Ray &ray, const HitInfo &hInfo, const LightList &lights, int bounceCount) const{ return Color(0, 0, 0); }
		virtual Color refractShade(const Ray &ray, const HitInfo &hInfo, const LightList &lights, int bounceCount) const{ return Color(0, 0, 0); }
	...
	public:
		...
		~TexturedColor() { if (map) delete map; }
