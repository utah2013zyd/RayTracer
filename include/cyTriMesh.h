...
inline bool cyTriMesh::LoadFromFileObj(const char *filename, bool loadMtl)
{
	...
	public:
		...
		void ReadVertex(cyPoint3f &v) const { sscanf(data + 2, "%f %f %f", &v.x, &v.y, &v.z); }
	...
	struct MtlData
	{
		char mtlName[256];
		int faceCount;
		MtlData() { mtlName[0] = '\0'; faceCount = 0; }
	};
	struct MtlList {
	...
	while (int rb = buffer.ReadLine(fp)) {
		if (buffer.IsCommand("v")) numVerts++;
		else if (buffer.IsCommand("vt")) numTVerts++;
		else if (buffer.IsCommand("vn")) numNormals++;
			...
			if (nFaceVerts > 2) {
				...
				if (currentMtlData) currentMtlData->faceCount += nFaceVerts - 2;
			}
		}
		else if (loadMtl) {
	...
	rewind(fp);
	while (int rb = buffer.ReadLine(fp)) {
		if (buffer.IsCommand("v")) buffer.ReadVertex(v[readVerts++]);
		else if (buffer.IsCommand("vt")) buffer.ReadVertex(vt[readTVerts++]);
		else if (buffer.IsCommand("vn")) buffer.ReadVertex(vn[readNormals++]);
		else if (buffer.IsCommand("f")) {
			int facevert = -1;
			bool inspace = true;
			int type = 0;
				...
				if (buffer[i] == ' ') inspace = true;
					...
					if (inspace) {
						...
						case -1:
							...
							if (ft) ft[rf].v[0] = ft[rf].v[1] = ft[rf].v[2] = 0;
							if (fn) fn[rf].v[0] = fn[rf].v[1] = fn[rf].v[2] = 0;
						case 0:
						case 1:
							facevert++;
							break;
					...
					if (buffer[i] >= '0' && buffer[i] <= '9') {
						...
						switch (type) {
						case 0: f[rf].v[facevert] = index - 1; break;
						case 1: if (ft) ft[rf].v[facevert] = index - 1; break;
						case 2: if (fn) fn[rf].v[facevert] = index - 1; break;
						}
					}
				}
			}
			rf++;
			if (currentMtlIndex >= 0) mtlFaceIndex[currentMtlIndex] = rf;
			else readFaces = rf;
		}
